...
class MQTTProtocol(Protocol, policies.TimeoutMixin):
    ...
    def dataReceived(self, data):
        self.setTimeout(
            GlobalObject().config.get("TIME_OUT_COUNT", MQTT_TIME_OUT)
        )
        logger.debug('received from target : {target} :'.format(
            target=(self.transport.client[0], self.transport.client[1])))
    ...
    def _event_connect(self, packet, qos, dup, retain):
        ...
        logger.debug("unpack the connect packet:{packet}".format(packet=packet))
    ...
    def _event_subscribe(self, packet, qos, dup, retain):
        # Extract the message ID
        logger.debug("subscribe packet:{packet}".format(packet=packet))
